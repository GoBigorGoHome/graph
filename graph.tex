...
\end{cases}
    ...
    若边$(u,v)\in E_f$满足$d(u)=d(v)+1$则称$(u,v)$为允许边，否则称为非允许边。若$G_f$的一条$s\leadsto t$路径$p$完全由可行边构成，则称$p$为允许路径。显然有
    \begin{property}
        可行路径是一条最短增广路。
    \end{property}

    \subsubsection*{SAP算法}
    SAP算法的思想与Dinic算法类似\footnote{这里所谓「类似」是指level($u$)与$d(u)$的意义有相似之处；但仍应注意层次图$G'_f$上的边和残余网络$G_f$中的可行边这两个概念的区别：若令$d(u)=\mathrm{level}(u)$，则层次图上的边一定是可行边；反过来不成立。}。确定一个初始的合法的距离标号，SAP算法重复下述过程，直到残量网络上不存在$s\leadsto t$路径（即增广路）：

    从$s$开始，沿着允许边DFS，试图到达$t$；若能到达$t$，则沿着找到的最短增广路增广，在新的残量网络上继续DFS。与Dinic算法不同的是，若DFS到$u$点之后无法继续前进，则增大$u$的距离标号，将$d(u)$更新为$\min\{d(v)+1\colon (u,v)\in E_f\}$，这一操作称作\emph{重标号}；然后回溯。下面给出SAP算法的代码：
