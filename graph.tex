...
\end{example}
  ...
  \begin{enumerate}
    ...
    subject to
    \begin{align}
      \sum_{v\colon(u,v)\in E} f(u,v) &- \sum_{v\colon(v,u)\in E}f(v,u) = b(i) \quad \forall u\in V,\\
      0\le f(u,v)&\le c(u,v)\quad \forall(u,v)\in E %TODO 让两个公式都居中
    \end{align}
  \end{enumerate}
  ...
  这一算法称作\emph{连续最短路算法}（successive shortest path algorithm）。
  % FIXME 坚持原创 ^-^
