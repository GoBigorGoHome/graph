...
\end{example}
  ...
  设$(S,T)$是网络$G'$的一个割，若边$(u,v)$满足$u\in S,v\in T$或$u\in T, v\in S$则称$(u,v)$为割边；若对任意割边$(u,v)$都有$u\in S$或$v\in T$则称$(S,T)$为简单割。不难证明，有向图$G$的闭包和网络$G'$的简单割一一对应。读者可自行验证，$G$的闭包$V_1$，对应于$G'$的简单割$(S,\bar{S})$，$S=\{s\}\cup V_1$；$G'$的简单割$(S,\bar{S})$对应于$G$的闭包$V_1$，$V_1=S-{s}$。

  我们来考虑简单割$(S,\bar{S})$的容量$f(S,\bar{S})$和$G$的闭包$V_1=S-s$的权值之间的关系。
  \section{费用流}
  \section{二分图}
  \subsection{最大流和二分图}
  \subsection{匈牙利算法}
