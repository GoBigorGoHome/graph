...
\end{cases}
  ...
  \begin{example}[Collector's Problem, UVa 10779]
    Bob和朋友们在收集糖果中的贴纸。为了让自己手中的贴纸种类尽量多，他们决定用自己的一张重复的贴纸去和别人交换一张自己所没有的贴纸。Bob意识到只跟别人交换自己所没有的贴纸并不总是最优的，在某些情况下，换来一张已有的贴纸更划算。
  ...
  用点$b_j$表示Bob的第$j$个朋友。若朋友$j$没有第$i$种贴纸，就从$a_i$向$b_j$连一条容量为$1$的边；若朋友$j$有$k$（$k\ge 2$）张第$i$种贴纸，就从$b_j$向$a_i$连一条容量为$k-1$的边。这样，最大流的值便是Bob所能获得的贴纸种类的最大值。
