...
\end{cases}
    ...
    \subsubsection*{小结}
    借助分层图这一概念，我们可以很直观地理解引理\ref{Lemma:1}。Dinic算法的过程就是不断的构造阻塞流，并用阻塞流来增广原来的流$f$；不难看出每次用阻塞流增广之后，残量网络上的最短$s\leadsto t$路径的长度严格递增，所以Dinic算法最多构造$n-1$次阻塞流，因而复杂度为$O(n^2m)$。这个复杂度上界是比较松的，Dinic算法的速度能满足大部分实际问题的要求。
