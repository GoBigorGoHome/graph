...
\end{cases}
    ...
    \subsection{ISAP算法}
    ISAP是Improved Shortest Augmenting Path的缩写。与Dinic算法一样，ISAP算法也是一种最短增广路算法；ISAP算法中引入了一些新的概念，下面一一介绍。
    \subsubsection*{距离标号}
    设$G(V,E,c,s,t)$是一个流网络，$f$是$G$上的一个流。在残量网络$G_f=(V,E_f)$上，我们定义一个距离函数$d\colon V\to\mathbb{N}$。若函数$d$满足下面两个条件则称$d$为合法的距离函数：
    \begin{enumerate}
        \item $d(t) = 0$;
